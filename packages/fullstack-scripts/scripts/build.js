...
process.env.BABEL_ENV = process.env.CUSTOM_ENV || 'production';
process.env.NODE_ENV = process.env.CUSTOM_ENV || 'production';
...
function buildProduction (previousFileSizes) {
  ...
  return build(prodConfig, 'production').then(res => {
...
function buildServerSide (previousFileSizes) {
  ...
  // Start the webpack build
  return build(ssrConfig, 'server side').then(res => {
...
function build(config, buildType) {
  console.log(`Creating an optimized ${buildType} build...`);
